...
<#
  v119+ (unchanged interface)
  - Accepts -Mode for YAML compatibility.
  - Sets MIG_* environment variables and runs discovery then business-mapping parameterless.
#>
param(
...
try {
  ...
  if ($SubscriptionId -and $SubscriptionId.Trim() -ne "") { Write-Info "Setting subscription context: $SubscriptionId"; az account set --subscription $SubscriptionId }
  ...
  $disc = Start-Process -FilePath 'powershell' -ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-File',(Resolve-Path $DiscoveryScriptPath).Path) -NoNewWindow -Wait -PassThru
  ...
  $map = Start-Process -FilePath 'powershell' -ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-File',(Resolve-Path $MappingScriptPath).Path) -NoNewWindow -Wait -PassThru
